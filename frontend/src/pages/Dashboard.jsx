  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        setLoading(true);
        setError('');
        const response = await client.get('/api/Admin/Summary');
  ...
  return (
    <div className="admin-dashboard">
      <div className="dashboard-header">
        ...
        <p className="dashboard-subtitle">Here's what's happening with your store</p>
      </div>
      ...
      {!loading && summary && (
        <div className="stat-grid">
          ...
          <div className="stat-card">
            <span className="stat-value">RS {summary.totalRevenue ?? 0}</span>
          ...
          <div className="stat-card highlight">
            <span className="stat-value">{summary.pendingOrders ?? 0}</span>
